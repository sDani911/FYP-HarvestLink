    ...
    return (
        <div className="text-white grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 min-h-screen h-full px-10 pb-11 ml-6 m-4">
            {filteredItems.map((item, index) => (
                <Link to={item.link}>
                    <div key={index} className="flex text-center flex-col justify-center border border-red-100 aspect-w-4 aspect-h-5 bg-blue-500 floating-box p-10">
                        <div className="h-[200] flex flex-col items-center just">
                            <img src={item.icon} alt={item.text} className="w-10 h-10 inline-block mr-2" />
                            <br />
                            {item.text}
                        </div>
                    </div>
